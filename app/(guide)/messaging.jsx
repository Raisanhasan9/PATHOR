...
// ─── Booking info banner ──────────────────────────────────────────────────────
function BookingBanner({ bookingId, travellerName }) {
  return (
    <View style={styles.banner}>
      ...
      <View style={styles.bannerText}>
        <Text style={styles.bannerTitle}>{travellerName || "Traveller"}</Text>
...
function Bubble({ item, myId }) {
  ...
  const messageText = item.content || item.message || item.text || "";
...
// ─── Main screen ──────────────────────────────────────────────────────────────
export default function GuideMessaging() {
  const router = useRouter();
  const { bookingId, travellerName, travellerId } = useLocalSearchParams();
  ...
  const fetchMessages = async (silent = false) => {
    ...
    try {
      ...
      console.error("GuideMessaging fetchMessages error:", err);
  ...
  const handleSend = async (overrideText) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setMessages(prev);
        setGrouped(groupByDate(prev));
        setInputText(text);
      }
    } catch (err) {
      console.error("GuideMessaging handleSend error:", err);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {/* ── Booking banner ── */}
        <BookingBanner bookingId={bookingId} travellerName={travellerName} />
        ...
        {loading ? (
          ...
          <View style={styles.centeredState}>
            ...
            <Text style={styles.stateText}>
              Your traveller hasn't sent any messages yet. You can reach out
              first.
            </Text>
          </View>
        ) : (
        ...
        <View style={styles.bottomSection}>
          <QuickReplies onSelect={(qr) => handleSend(qr)} />
...
const styles = StyleSheet.create({
  ...
  orbTop: {
    ...
    top: -width * 0.62,
    left: -width * 0.05,
    backgroundColor: "rgba(5,120,80,0.15)",
  },
  ...
  bannerSub: { color: T.muted, fontSize: 11, marginTop: 1 },
  bannerBadge: {
    flexDirection: "row",
    alignItems: "center",
    gap: 5,
  ...
  bannerBadgeText: { color: T.text3, fontSize: 10, fontWeight: "700" },
  ...
  stateEmoji: { fontSize: 46, marginBottom: 4 },
  stateTitle: {
    color: T.text,
    fontSize: 17,
    fontWeight: "700",
  ...
  dateDividerLine: { flex: 1, height: 1, backgroundColor: T.border },
  dateDividerText: {
    color: T.muted,
    fontSize: 11,
    fontWeight: "600",
  ...
  bubbleText: { color: T.text2, fontSize: 14.5, lineHeight: 21 },
  ...
  quickChipText: { color: T.text3, fontSize: 12, fontWeight: "600" },
  quickToggle: {
    paddingHorizontal: 14,
    paddingVertical: 8,
  },
